...
#define BBR_SCALE 4	/* scaling factor for fractions in BBR (e.g. gains) */
...
/* Maximum size for send buffer declared would be bbr_sndbuf_scale * cwnd: */
static const int bbr_sndbuf_scale = 6;
/* Minimum value of TCP_INIT_CWND */
static const u32 bbr_min_init_cwnd = 15;
...
static u32 bbr_target_cwnd(struct sock *sk, u32 bw, int gain)
{
	...
	if (unlikely(bbr->min_rtt_us == ~0U))	 /* no valid RTT samples yet? */
		return max_t(u32, TCP_INIT_CWND, bbr_min_init_cwnd);  /* be safe: cap at default initial cwnd*/
	...
	struct sock *sk, const struct rate_sample *rs, u32 acked, u32 *new_cwnd)
{
	 ...
	 */
	if (rs->losses > 0)
		cwnd = max_t(s32, cwnd - rs->losses, bbr_cwnd_min_target);
	...
	if (state == TCP_CA_Recovery && prev_state != TCP_CA_Recovery) {
		...
		cwnd = max(tcp_packets_in_flight(tp) + acked, bbr_cwnd_min_target + acked);
			 ...
			 u32 acked, u32 bw, int gain)
{
	...
	else if (cwnd < target_cwnd || tp->delivered < max_t(u32, TCP_INIT_CWND, bbr_min_init_cwnd))
		cwnd = cwnd + acked;
	...
	tp->snd_cwnd = max(cwnd * 3 / 4, bbr_cwnd_min_target + acked);
}
